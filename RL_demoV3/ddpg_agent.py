...
class Agent():
    ...
    def choose_action(self, observation):
        ...
        # Gausian noise is used instead of Ornstein–Uhlenbeck for simplicity
        gausianNoise = np.random.normal(0, 0.5, 3)*0.5  # mean, standart deviation and size 
        action_noise = action + T.tensor(gausianNoise,
                                         dtype=T.float).to(self.actor.device)
        return action_noise.cpu().detach().numpy()[0]  
